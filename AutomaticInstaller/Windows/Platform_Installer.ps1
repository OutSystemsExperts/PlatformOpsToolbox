...
$FileDevTools = Get-ChildItem $InstallerScriptPath\bin\*.exe  | Where-Object {("{0}" -f [System.Diagnostics.FileVersionInfo]::GetVersionInfo($_).FileDescription) -match "Development Environment Installer"} -ErrorAction SilentlyContinue
        ...
        if($FileDevTools){
                ...
                try{
                    ...
                    Start-Process $FileDevTools -ErrorAction SilentlyContinue -ArgumentList "/S /v/qn” -Wait
...
$FilePlatformServer = Get-ChildItem $InstallerScriptPath\bin\*.exe  | Where-Object {("{0}" -f [System.Diagnostics.FileVersionInfo]::GetVersionInfo($_).FileDescription) -match "Platform Server Installer"} -ErrorAction SilentlyContinue
        ...
        if($FilePlatformServer){
                ...
                try{
                    ...
                    Start-Process $FilePlatformServer -ErrorAction SilentlyContinue -ArgumentList "/S /v/qn” -Wait
        ...
        Finally{
           ...
           Do{
                ...
                if(!$User){
                    ...
                    Write-Host "Please insert username!"
            ...
            Do{
                ...
                [string]$UserPwd = Read-Host "Username password"

                if(!$UserPwd){
            
                    Write-Host "Please insert username password!"
            ...
            $Directory = $OS.'(default)'
            $ps = new-object System.Diagnostics.Process
            $ps.StartInfo.Filename = "$Directory\OSPTool.exe"
            $ps.StartInfo.RedirectStandardOutput = $True
            $ps.StartInfo.RedirectStandardError = $True
            $ps.StartInfo.UseShellExecute = $false
            $ps.EnableRaisingEvents = $True
            
           $ps.StartInfo.Arguments = "/Publish $Directory\System_Components.osp $env:computername $DBAdmin $DBAdminPwd"
           
           $ps.Start() | Out-Null -ErrorAction Stop
            Write-Host "Systems Components will be published." -ForegroundColor Yellow       
            Write-Host "`nThis process may take a while.`nPlease check Service Center" -ForegroundColor Yellow
