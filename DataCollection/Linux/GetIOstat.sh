...
DESCRIPTION
  This script gets all IO statistics from all detected devices with activity
...
if ps -ef | grep -v grep | grep iostat ; then
        exit 0
else
...
OUTPUT=$(/usr/bin/iostat -dtx "$INTERVALS" > /"$DIR"/iostat-"$HOSTNAME"-"$DATEVAR".out)
fi
