...
if($OSVersion -eq "2008" -or $OSVersion -eq "2012")
{
 ...
 if ($PlatformVersion -eq "9.1" )
  ...
  $AlwaysWithoutDS = Invoke-Command -ComputerName $ComputerName {Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\MSMQ\Parameters\Setup\" | Select-Object -ExpandProperty "AlwaysWithoutDS"} -ErrorAction SilentlyContinue
